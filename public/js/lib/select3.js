...
'use strict';
selectNilaoweiAbc.prototype = {
	constructor: selectNilaoweiAbc
	/*
	 //选中树节点
	 //$.选中的节点数据
	 */
	...
	,bindChoiceEvent: function(){
		...
		$('.'+ _this.domMark).on('click', '.choices-list, .search-input', function(e){
			...
			_this.outLog('selectNilaoweiAbc:打开下拉列表事件');
			...
			if(!_thisDOM.hasClass('fc-doing')){
				...
				if(!_this.isMultiple) _choicesArea.hide();
		...
		$('body').bind('click', function(e){
			...
			_this.outLog('selectNilaoweiAbc:关闭下拉列表');
		...
		$('.'+ _this.domMark).on('click', '.choices-action', function(e, _action_){
			...
			_this.outLog('selectNilaoweiAbc:选中事件');

			_thisDOM		= _title.parents('.'+ _this.domMark).eq(0);
			_list			= _title.parent().find('> .list-element');
			_memory 		= _thisDOM.parent().find('[selectNilaoweiAbc-id="tas_' + _thisDOM.attr('tas')+'"]');
			_choicesArea = _thisDOM.find('.choices-area');
			_choicesList 	= _thisDOM.find('.choices-list');
			...
			if(_list.length == 0 || _this.isSelectParent){
				...
				_showHtml = '<li class="choice-field" node-id="'+ _title.attr('node-id') +'">'
					...
					+ '</li>';
				if (!_this.isMultiple && _memory.val().length > 0) {
					_choicesList.find('li:first-child .del-action').click()
					console.log('单选')
					//return;
				}
		...
		$('.'+ _this.domMark).on('click', '.del-action', function(){
			...
			_this.outLog('selectNilaoweiAbc:鼠标关闭事件触发');
			//移除存储数据	:val
		...
		$('.'+ _this.domMark).on('keydown', '.search-input', function(e){
			...
			if(_searchText.length == 0 && e.keyCode === 8){
				_this.outLog('selectNilaoweiAbc:键盘回退事件触发');
		...
		$('.'+ _this.domMark).on('keyup', '.search-input', function(e){
			...
			_this.outLog('selectNilaoweiAbc:搜索事件');
	...
	,createDOM: function(dom){
		var _this = this;
		var _dom = $(dom);
		var _warpHtml = '<div style="position:absolute;z-index:999" class="tree-type-search '+ _this.domMark +'">'
			...
			+ '<div class="search_box"><input type="text" class="search-input" record="" placeholder="'+_this.placeholder+'"/><span class="icon-top-t1"></span></div>'
			+ '<div class="tree-area">'
			 
			+ '</div>'
			+ '<i class="icon-loading icon-down-s1"></i>'
			+ '</div>';
		var _warp,
			_choicesList;
		//生成树
		...
		//配置原生DOM节点
		_dom.attr('selectNilaoweiAbc-id', 'tas_' + _dom.attr("name") + '_' + _this.spanningOnly(_dom.attr("name")));
		_dom.val('');
		_dom.hide();
		...
		_warp.find('.icon-top-t1').click(function(){
			$('body').trigger('click')
		})
		...
		function spanning(treeData, pId){
			...
			$.each(treeData, function(i, v){
				...
				treeHtml += '<span class="ts-icon icon-right-s1 choices-action" style="width:'+ (layer+1) * _this.indent+'px;'
					...
					+ (layer + 1) * _this.indent +'px" node-id="'+(v.nodeId || v.name)+'" title="'+ v.name +'"'
				...
				if(isHaveSon){
					layer++;
					spanning(v.son, (v.nodeId || v.name));
					layer--;
				}
				treeHtml += '</li>';
			});
...
function selectNilaoweiAbc(node, s1, s2){
	var setting = {};
	this.domMark			= 'tree-type-search';	//标识，用于生成下拉框
	this.treeData			= [];					//树源数据
	this.data				= [];					//选中的节点
	this.animateTime		= 0;					//滑动动画耗时
	...
	this.height				= '24px';				//控件展示高度
	this.matchLimit 		= 999999;					//搜索时单次进行匹配的量，该值越大匹配越快，但是会影响性能，
	this.matchTime  		= 1;					//搜索时单次进行匹配的延时，该值越小匹配越快，但是会影响性能
	this.isDevelopMode     	= false;				//是否为开发模式，为true时将打印事件日志
	this.placeholder		= '请输入手机标签';	//选中项为空的占位符

	if(typeof(s1) == 'string' && !s2){
		throw new Error('selectNilaoweiAbc参数错误，请参考使用文档');
		return false;
	}
	...
	if(node.parent().find('.' + this.domMark).length == 0){
		this.createDOM(node);//创建selectNilaoweiAbc DOM
...
(function(){
	...
	$.fn.selectNilaoweiAbc = function(s1, s2){
		var sel = new selectNilaoweiAbc(this, s1, s2);
	};
})();
