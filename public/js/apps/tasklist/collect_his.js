...
function AddEventBtn(a,value,c){
  return  [ '<button id="check_res"  type="button" class="btn" >关注任务</button> ', ].join('');
 
}
var btn_events = {
  "click #check_res" : function(a,b,value){
      var type_data = {
        task_name:$('#tb').find('input[name=task_name]').val(),
        class_name:$('#tb').find('input[name=class_name]').val()
      }

      parent.mainPlatform.show_child_iframe('apps/tasklist/concern_his.html',value.name+'的任务详情','taskDetail_'+value.id,false,false,value,'collect_his',type_data)
  },

}
function search_init(){

  var newData = window.taskDetailInfo ; 
   if(newData){
    if(newData.showIframe == 'collect_his'){

       window.taskDetailInfo['showIframe'] = ''
       $("#tb").find('input[name="task_name"]').val(newData.showIframe_data['task_name'])
       $("#tb").find('input[name="class_name"]').val(newData.showIframe_data['class_name'])
    }
  ...
  $("#timeSel").selectpicker('refresh');
      ...
      function queryParams(params){
        ...
        var data = { //每页多少条数据
          ...
          "class_id" :node.find('input[name=class_name]').val(),
